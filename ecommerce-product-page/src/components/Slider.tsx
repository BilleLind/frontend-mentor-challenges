...
export default ({ slides }: SlidesProp) => {
	...
	return (
		<section className="w-full h-60 relative">
			<button onClick={prevSlide} className={`absolute left-0 inset-y-1/2 translate-t-1/2 z-20 bg-white rounded-full w-6 h-6 inline-flex items-center justify-center ${current ===0 ? 'hidden': ''}`}>
				<IconPrevious className="-ml-1" />
			</button>
			<button onClick={nextSlide} className={`absolute right-0 top-1/2 translate-t-1/2 z-20 bg-white rounded-full w-6 h-6 inline-flex items-center justify-center ${current ===length-1 ? 'hidden': ''}`}>
